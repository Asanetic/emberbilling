...
export async function POST(SubscriptionaccountsRequest) {
  try {
    ...
    const SubscriptionaccountsFormAction = body.plans_mosy_action;
  ...
  //--- End plans inputs array --//
    ...
    const mutatedDataArray =mutateInputArray('plans',SubscriptionaccountsInputsArr, SubscriptionaccountsRequest, newId, authData)

    if (SubscriptionaccountsFormAction === "add_plans") 
    {
    ...
    if (SubscriptionaccountsFormAction === "update_plans") {
      
      // update table Subscriptionaccounts
      const result = await UpdateSubscriptionaccounts(newId, mutatedDataArray, body, authData, `primkey='${plans_uptoken_value}'`)
